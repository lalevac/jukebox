...
spotify.shouldAskUserCredentials = () => {
  if (spotify.mustAuthenticate) { console.log('We must ask the user for authentication!') } else { console.log('No need to ask the user for authentication') }
...
const writeTokenToDisk = (tokenName, tokenContent) => {
  fs.writeFile(tokenName, tokenContent, (err) => {
    if (err) { console.error('Could not write "' + tokenName + '" token to disk D: Err: ' + err) } else { console.log('Successfully wrote "' + tokenName + '" token to disk.') }
  })
}
